...
{   
    ...
    {
        global $_dbh;
        $sql="CAll module_master_search('t.module_name, t.module_text, t.tab_index, t.module_id','tbl_module_master t')";
        echo "
        ...
        <thead>
            ...
            <th> Module Name <br><input type=\"text\" data-index=\"1\" placeholder=\"Search Module Name\" /></th> 
                         <th> Module Text <br><input type=\"text\" data-index=\"2\" placeholder=\"Search Module Text\" /></th> 
                         <th> Tab Index <br><input type=\"text\" data-index=\"3\" placeholder=\"Search Tab Index\" /></th> 
                         </tr>
        </thead>
        <tbody>";
         $_grid="";
        ...
        $_grid.="<tr>
        <td> 
            <form  method=\"post\" action=\"frm_module_master.php\" style=\"display:inline; margin-rigth:5px;\">
            ...
            </form> <form  method=\"post\" action=\"classes/cls_module_master.php\" style=\"display:inline;\">
 ...
 class dal_modulemaster                         
{
    public function dbTransaction($_mdl)                     
    {
        ...
        $_pre->bindParam(1,$_mdl->_module_name);
        $_pre->bindParam(2,$_mdl->_module_text);
        $_pre->bindParam(3,$_mdl->_tab_index);
    ...
    public function fillModel($_mdl)
    {
        ...
        $_mdl->_module_name=$_rs[0]["module_name"];
        $_mdl->_module_text=$_rs[0]["module_text"];
...
if(isset($_POST["masterHidden"]) && ($_POST["masterHidden"]=="save"))
{
            ...
            if(isset($_REQUEST["module_name"]) && !empty($_REQUEST["module_name"]))
                $field=trim($_REQUEST["module_name"]);
            else {
                    $field=null;
           }
            $_bll->_mdl->_module_name=$field;
            if(isset($_REQUEST["module_text"]) && !empty($_REQUEST["module_text"]))
                $field=trim($_REQUEST["module_text"]);
            else {
                    $field=null;
           }
            $_bll->_mdl->_module_text=$field;
