                           ...
                           <?php
                                if (!empty($menuData)) {
                                    ...
                                    foreach ($menuData as $module => $groups) {
                                        ...
                                        echo "<div class='card-body' id='menus_$module' style='display:none;'>";

                                        foreach ($groups as $group => $menuItems) {
                                            echo "<div class='menu-group' id='group_$group'>";

                                            foreach ($menuItems as $item) {
                                                $menuRightId = isset($item['menu_right_id']) ? $item['menu_right_id'] : '';

                                                $viewRight = isset($_bll->_mdl->_array_detail[$menuRightId]['view']) ? $_bll->_mdl->_array_detail[$menuRightId]['view'] : 0;
                                                $addRight = isset($_bll->_mdl->_array_detail[$menuRightId]['add']) ? $_bll->_mdl->_array_detail[$menuRightId]['add'] : 0;
                                                $editRight = isset($_bll->_mdl->_array_detail[$menuRightId]['edit']) ? $_bll->_mdl->_array_detail[$menuRightId]['edit'] : 0;
                                                $deleteRight = isset($_bll->_mdl->_array_detail[$menuRightId]['delete']) ? $_bll->_mdl->_array_detail[$menuRightId]['delete'] : 0;

                                                echo "<div class='row'>";

                                                echo "<div class='col-md-2 menu-name-column'><span class='menu-name'>{$item['name']}</span></div>";

                                                echo "<input type='hidden' name='rights[{$item['name']}][menu_right_id]' value='" . htmlspecialchars($menuRightId) . "' />";
