...
document.getElementById('saveSelectedInward').addEventListener('click', function() {
    ...
    // Create new records from checked rows
    let newRecords = [];
    checkedRows.forEach(cb => {
        const tr = cb.closest('tr');
        const record = {
    ...
    // Remove duplicates based on inward_detail_id
    const existingIds = new Set(window.jsonData.map(item => item.inward_detail_id));
    newRecords = newRecords.filter(record => !existingIds.has(record.inward_detail_id));

    // Append new records to jsonData
    window.jsonData = [...window.jsonData, ...newRecords];

    // Clear "No records available" row if present
    const noRecordsRow = document.getElementById('norecords');
    if (noRecordsRow) {
        noRecordsRow.remove();
    }

    // Update the table by appending new rows
    newRecords.forEach((data, idx) => appendTableRow(data, window.jsonData.length - newRecords.length + idx));

    // Update the hidden input with the merged jsonData
    const detailsInput = document.getElementById('selected_inwards_json');
    if (detailsInput) {
        detailsInput.value = JSON.stringify(window.jsonData);
    }

    // Hide the modal
    bootstrap.Modal.getInstance(document.getElementById('modalDialog')).hide();

    // Disable customer select after selection
...
$(document).ready(function () {
    ...
    start_date: '<?php echo $yearRow['start_date'] ?? ''; ?>',
    end_date: '<?php echo $yearRow['end_date'] ?? ''; ?>'
};

// Store the auto-filled date
let autoFilledDate = null;

// Function to set default dates based on company_year_id
function setDefaultOutwardDate() {
    const outwardDateInput = $('#outward_date');
    const today = new Date();
    let defaultDate = new Date(today);

    // Get the financial year's start and end dates
    const startDate = new Date(companyYearDetails.start_date);
    const endDate = new Date(companyYearDetails.end_date);

    // Set the year to the financial year's start year
    defaultDate.setFullYear(startDate.getFullYear());

    // (Optional) if you want to set to last day of current month if day mismatches
    ...
    autoFilledDate = formattedDate;
    validateOutwardDate();
}

function validateOutwardDate() {
    const outwardDateInput = $('#outward_date');
    const errorContainer = $('#outward_date_error');
    const outwardDate = outwardDateInput.val();

    // Clear previous errors
    outwardDateInput.removeClass('is-invalid');
    errorContainer.text('');
    ...
    // Parse the date
    const selectedDate = new Date(outwardDate);
    ...
    const minAllowedDate = new Date(autoFilledDateObj);
    minAllowedDate.setDate(minAllowedDate.getDate() - 30);

    // Validate date range
    if (selectedDate < minAllowedDate) {
        showError(`Date below current period (${autoFilledDate})`);
        return false;
    }
    if (selectedDate > autoFilledDateObj) {
        showError(`Date above current period (${autoFilledDate})`);
        return false;
    }
    ...
    function showError(message) {
        ...
        outwardDateInput.addClass('is-invalid');
    }
}

// Handle company_year_id change
$('#company_year_id').on('change', function() {
    const companyYearId = $(this).val();
    if (companyYearId) {
        $.ajax({
            url: 'classes/cls_outward_master.php',
            type: 'POST',
            data: {
                action: 'fetchCompanyYearDetails',
            ...
            success: function(response) {
                try {
                    ...
                    if (data.success) {
                        companyYearDetails.start_date = data.start_date;
                        companyYearDetails.end_date = data.end_date;
                        setDefaultOutwardDate();
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Failed to fetch company year details.'
                        });
                    }
                } catch (e) {
                    Swal.fire({
            ...
            error: function() {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Error fetching company year details.'
                });
            }
        });
    }
...
// Initialize default date and validation
setDefaultOutwardDate();
$('#outward_date').on('blur', validateOutwardDate);
});
</script>
