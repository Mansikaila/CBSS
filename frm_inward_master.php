...
?>
  ...
  <div class="content-wrapper">
    ...
    <?php
     ...
     $value = "";
if ($fields_names[$i] == "unloading_charge") {
    // Set initial value from unloading_charge_amount (if available)
    $value = isset($_POST['unloading_charge_amount']) ? $_POST['unloading_charge_amount'] : 
             (isset($_bll->_mdl->$cls_field_name) ? $_bll->_mdl->$cls_field_name : "");
} else {
...
$(document).ready(function() {
    ...
    let pendingDuplicateChecks = 0; 
    let isUnloadingChargeManuallyEdited = false;
    // DOM elements
    ...
    const inwardNoHidden = document.getElementById("inward_no_hidden");
        ...
        if (!row.classList.contains("norecords")) {
            ...
            row.querySelectorAll("td[data-label]").forEach(td => {
                if (!td.classList.contains("actions")) {
                    ...
                    if (td.dataset.label === "gst_type" && td.dataset.gst_type_id) {
                    rowData["gst_type"] = td.dataset.gst_type_id; // Store the ID
                    rowData["gst_type_value"] = td.innerText; // Store the value
                }
                }
            });
    ...
    function checkDuplicate(input, callback) {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                console.error(`Duplicate check failed for ${input.name}: ${error}`);
                input.classList.remove("is-invalid");
                const errorContainer = input.nextElementSibling;
                if (errorContainer && errorContainer.classList.contains("invalid-feedback")) {
                    errorContainer.textContent = "";
    ...
    if (inwardSequenceInput && inwardNoInput) {
        inwardSequenceInput.addEventListener("input", function () {
            ...
            if (inwardNoHidden) {
                inwardNoHidden.value = inwardNoInput.value;
            }
        });
    }
    ...
    window.openModal = function(index = -1) {
        ...
        if (data['gst_type'] !== undefined && data['gst_type'] !== null) {
    const gstRadios = form.querySelectorAll('input[name="gst_type"]');
    gstRadios.forEach(radio => {
        radio.checked = (radio.value === String(data['gst_type']));
    });
}
            ...
            if (data['location'] !== undefined && data['location'] !== null) {
                ...
                if (locationParts.length >= 3) {
                    ...
                    if (rackInput) rackInput.value = locationParts[2].trim();
                }
            }
            ...
            const unloadingChargeAmountInput = document.getElementById('unloading_charge_amount');
            const unloadingChargeInput = document.getElementById('unloading_charge');
            if (unloadingChargeAmountInput && unloadingChargeInput) {
                // Sync initial value
                unloadingChargeInput.value = unloadingChargeAmountInput.value;

                // Update unloading_charge when unloading_charge_amount changes
                unloadingChargeAmountInput.addEventListener('input', function () {
                    unloadingChargeInput.value = this.value;
                    isUnloadingChargeManuallyEdited = true;
                });

                // Update unloading_charge_amount when unloading_charge changes
                unloadingChargeInput.addEventListener('input', function () {
                    unloadingChargeAmountInput.value = this.value;
                    isUnloadingChargeManuallyEdited = true;
                });
            }
            ...
            for (let key in data) {
                if (key === "item" || key === "packing_unit" || key === "location" || key === "seasonal_rent_per" || key === "unloading_charge") continue;
            ...
            document.getElementById("detailtransactionmode").value = "I";
            conversionFactor = 0;
            // Clear unloading_charge and unloading_charge_amount
            const unloadingChargeInput = document.getElementById('unloading_charge');
            const unloadingChargeAmountInput = document.getElementById('unloading_charge_amount');
            if (unloadingChargeInput) unloadingChargeInput.value = "";
            if (unloadingChargeAmountInput) unloadingChargeAmountInput.value = "";
...
function saveData() {
    ...
    const selectedGstType = form.querySelector('input[name="gst_type"]:checked');
    newEntry['gst_type'] = selectedGstType ? selectedGstType.value : "";
    newEntry['gst_type_value'] = selectedGstType ? selectedGstType.parentElement.textContent.trim() : "";
    
    // Add unloading_charge to newEntry
    const unloadingChargeAmountInput = document.getElementById('unloading_charge_amount');
    newEntry['unloading_charge'] = unloadingChargeAmountInput ? unloadingChargeAmountInput.value : "";
    ...
    function appendTableRow(rowData, index) {
        ...
        const displayFields = [
            'lot_no', 'item_name', 'gst_type_value', 'variety', 'packing_unit_name', 
        ...
        displayFields.forEach(field => {
            ...
            if (field === 'gst_type_value') {
            cell.setAttribute("data-gst_type_id", rowData['gst_type'] || "");
        }
    ...
    function rebuildTable() {
        ...
        if (jsonData.length === 0) {
            ...
            noRecordsCell.colSpan = 15; // Updated to account for unloading_charge column
...
function calculateTotals() {
    ...
    jsonData.forEach(row => {
        ...
        totalQty += qty;
        totalWt += wt;
        totalUnloadingCharge += unloadingCharge;
    });
    ...
    if (totalUnloadingChargeInput) {
        totalUnloadingChargeInput.value = totalUnloadingCharge.toFixed(2);
    }
    
    // Update unloading_charge only if not manually edited
    const unloadingChargeInput = document.getElementById('unloading_charge');
    if (unloadingChargeInput && !isUnloadingChargeManuallyEdited) {
        unloadingChargeInput.value = totalUnloadingCharge.toFixed(2);
    }
}
    ...
    document.getElementById("btn_add").addEventListener("click", async function(event) {
        ...
        masterForm.querySelectorAll(":invalid").forEach(input => {
            input.classList.add("is-invalid");
            if (input.nextElementSibling && input.nextElementSibling.classList.contains("invalid-feedback")) {
                input.nextElementSibling.textContent = input.validationMessage;
            }
            if (!firstRequiredField) firstRequiredField = input;
        });

        // Duplicate check
        const duplicateInputs = document.querySelectorAll(".duplicate");
        const duplicateCheckResults = await Promise.all(
            Array.from(duplicateInputs).map(input => {
                return new Promise(resolve => {
                    let column_value = input.value.trim();
                    if (column_value === "") { resolve({input, isDuplicate: false}); return; }
                    let id_column = "inward_id";
                    let id_value = document.getElementById(id_column)?.value || "0";
                    $.ajax({
                        url: "classes/cls_inward_master.php",
                        type: "POST",
                        data: {
                        ...
                        success: function(response) {
                            ...
                            if (result === 1) {
                                ...
                                if (!firstDuplicateField) firstDuplicateField = input;
                                resolve({input, isDuplicate: true});
                            } else {
                                ...
                                resolve({input, isDuplicate: false});
                            }
                        },
                        error: function() {
                            resolve({input, isDuplicate: false});
        ...
        if (firstDuplicateField) {
            ...
            firstDuplicateField.scrollIntoView({behavior: "smooth", block: "center"});
            return false;
        } else if (firstRequiredField) {
...
if (packingUnitDropdown) {
            ...
            fetch("classes/cls_inward_master.php?action=fetchPackingUnitData&packing_unit=" + encodeURIComponent(selectedPackingUnit))
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Update unloading_charge_amount and unloading_charge
                        const unloadingChargeAmountInput = document.getElementById("unloading_charge_amount");
                        if (unloadingChargeAmountInput) {
                            unloadingChargeAmountInput.value = data.unloading_charge || "";
                        }
                        // Only update unloading_charge if not manually edited
                        const unloadingChargeInput = document.getElementById("unloading_charge");
                        if (unloadingChargeInput && !isUnloadingChargeManuallyEdited) {
                        ...
                        // Clear unloading_charge_amount
                        const unloadingChargeAmountInput = document.getElementById("unloading_charge_amount");
                        if (unloadingChargeAmountInput) {
                            unloadingChargeAmountInput.value = "";
                        }
                        // Only clear unloading_charge if not manually edited
                        const unloadingChargeInput = document.getElementById("unloading_charge");
                        if (unloadingChargeInput && !isUnloadingChargeManuallyEdited) {
    ...
    // Real-time sync for unloading_charge_amount and unloading_charge
    const unloadingChargeAmountInput = document.getElementById('unloading_charge_amount');
    const unloadingChargeInput = document.getElementById('unloading_charge');
    if (unloadingChargeAmountInput && unloadingChargeInput) {
        // Sync initial value
        ...
        // Optional: Update unloading_charge_amount when unloading_charge changes (two-way sync)
        // unloadingChargeInput.addEventListener('input', function () {
        //     unloadingChargeAmountInput.value = this.value;
        // });
    }
if (unloadingChargeInput) {
    ...
    // Billing and Inward Date Validation
    if ($('#inward_date').val() === '') {
        var today = new Date();
    ...
    // Rent Per and Storage Duration Calculations
