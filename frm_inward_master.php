...
function checkDuplicate(input, callback) {
    ...
    $.ajax({
        ...
        success: function(response) {
            //console.log(`Duplicate check for ${input.name}: ${response} (value: ${column_value})`);
    ...
    if (inwardSequenceInput && inwardNoInput) {
        inwardSequenceInput.addEventListener("input", function () {
            ...
            // DO NOT call checkDuplicate here!
        });
    }
    ...
    document.getElementById("btn_add").addEventListener("click", async function(event) {
        ...
        if (!masterForm.checkValidity()) {
            ...
            const firstInvalid = masterForm.querySelector(":invalid");
            if (firstInvalid) firstInvalid.focus();
            return false;
        } else {
            masterForm.querySelectorAll(".is-invalid").forEach(input => {
                input.classList.remove("is-invalid");
                if (input.nextElementSibling) {
        ...
        // Only here - run duplicate check for all duplicate fields
        ...
        for (const result of duplicateCheckResults) {
            if (result.isDuplicate) {
                hasDuplicate = true;
                if (!firstDuplicateField) firstDuplicateField = result.input;
            }
        }
