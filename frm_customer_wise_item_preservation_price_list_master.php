  ...
  $(document).ready(function () {
    ...
    function fetchDetailRecords(customerId, itemId, masterId) {
        $.ajax({
            ...
            success: function(response) {
                ...
                // If editing existing record, initialize the detail_records field
                if (masterId > 0) {
            ...
            error: function() {
                $('#searchDetail .box-detail').html('<div class="alert alert-danger">Failed to load detail records.</div>');
            }
        });
    }

function checkMasterRecord(customerId, itemId, masterId = 0) {
    $.ajax({
        ...
        success: function(response) {
            if (response.exists) {
                if (response.is_current || masterId === 0) {
                    // Allow editing existing record or creating new with same combo if masterId=0
                    fetchDetailRecords(customerId, itemId, masterId);
                } else {
...
$('#item_id').on('change', function() {
    ...
    checkMasterRecord(customerId, itemId, masterId);
});
 ...
 $('#item_id, #customer_id').on('change', function () {
    ...
    // In Add mode, don't check for existing master record - just fetch units
    if (transactionMode === 'I') {
        resetMasterForm();
        fetchDetailRecords(customerId, itemId, 0);
    }
    // In Edit mode, we handle this on page load
});
...
function collectDetailRecords() {
    ...
    gridRows.each(function() {
        ...
        // Include all records that have values or have an ID (existing records)
    ...
    // ---- MOST IMPORTANT: On Edit mode, fetch master and detail by master id on page load ----
    ...
    if (transactionMode === 'U' && masterId && itemId && customerId) {
        // Edit mode: fetch both master and detail records
