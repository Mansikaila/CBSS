...
class bll_userrightmaster {
    ...
    public function processRights() {
        foreach ($_POST['rights'] as $menuName => $rights) {
            foreach ($rights as $rightType => $value) {
                $this->_mdl->_array_detail[$menuName][$rightType] = ($value === $rightType) ? 1 : 0;
            }
        }

        foreach ($this->_mdl->_array_detail as $menuRightId => $rights) {
            if (empty($rights['view']) && empty($rights['add']) && empty($rights['edit']) && empty($rights['delete'])) {
                $this->_mdl->_array_deleted[] = $menuRightId;
            }
        }
    }
